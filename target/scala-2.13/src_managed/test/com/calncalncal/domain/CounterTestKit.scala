...
final class CounterTestKit private(entity: Counter) {
  ...
  def increase(command: calncalncal.IncreaseValue): ValueEntityResult[Empty] = {
    val effect = entity.increase(state, command)
    interpretEffects(effect)
  }

  def decrease(command: calncalncal.DecreaseValue): ValueEntityResult[Empty] = {
    val effect = entity.decrease(state, command)
    interpretEffects(effect)
  }

  def reset(command: calncalncal.ResetValue): ValueEntityResult[Empty] = {
    val effect = entity.reset(state, command)
    interpretEffects(effect)
  }

  def getCurrentCounter(command: calncalncal.GetCounter): ValueEntityResult[calncalncal.CurrentCounter] = {
    val effect = entity.getCurrentCounter(state, command)
    interpretEffects(effect)
  }
}
