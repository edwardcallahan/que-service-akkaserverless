...
class CounterRouter(entity: Counter) extends ValueEntityRouter[CounterState, Counter](entity) {
  def handleCommand(commandName: String, state: CounterState, command: Any, context: CommandContext): ValueEntity.Effect[_] = {
    commandName match {
      case "Increase" =>
        entity.increase(state, command.asInstanceOf[calncalncal.IncreaseValue])

      case "Decrease" =>
        entity.decrease(state, command.asInstanceOf[calncalncal.DecreaseValue])

      case "Reset" =>
        entity.reset(state, command.asInstanceOf[calncalncal.ResetValue])

      case "GetCurrentCounter" =>
        entity.getCurrentCounter(state, command.asInstanceOf[calncalncal.GetCounter])
